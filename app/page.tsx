...
export default function Dashboard() {
  ...
  const handleStartDateChange = (date: Date | null) => {
    ...
    if (selectedAccount && date) {
      localStorage.setItem(`timeline-start-date-${selectedAccount.id}`, date.toISOString())
    }
  }
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="max-w-2xl mx-auto text-center space-y-8">
            <div className="space-y-4">
              ...
              <h1 className="text-3xl font-bold tracking-tight">Account Management Dashboard</h1>
              <p className="text-muted-foreground text-lg">
                Select or create an account to start tracking 8-week progress timelines
              </p>
            </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Create your first account to begin managing progress timelines</CardDescription>
              </CardHeader>
              <CardContent>
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Fixed Progress Bar */}
      <div className="fixed top-0 left-0 right-0 z-50 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b border border-red-600 mb-10">
        ...
        <div className="container mx-auto px-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h1 className="text-3xl font-bold tracking-tight">
                  {selectedAccount ? `${selectedAccount.name} - 8-Week Timeline` : "8-Week Timeline Dashboard"}
                </h1>
                <p className="text-muted-foreground mt-2">
                  Track progress through an 8-week journey of goal achievement
                </p>
              </div>
              {selectedAccount && <StartDatePicker startDate={startDate} onStartDateChange={handleStartDateChange} />}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">Across all weeks</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold">{goals.filter((goal) => goal.completed).length}</div>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  ...
                  {startDate
                    ? Math.min(
                        8,
                        Math.max(1, Math.floor((Date.now() - startDate.getTime()) / (7 * 24 * 60 * 60 * 1000)) + 1),
                      )
                    : "-"}
                </div>
                <p className="text-xs text-muted-foreground">Current focus period</p>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <p className="text-sm text-muted-foreground">Click on any week to manage goals</p>
            </div>
          ...
          <div className="mt-12">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Get started with {selectedAccount.name}'s 8-week journey</CardDescription>
              </CardHeader>
                ...
                {!startDate && (
                  <div className="flex-1 p-4 border border-dashed rounded-lg">
                    ...
                    <p className="text-sm text-muted-foreground mb-3">
                      Choose when {selectedAccount.name}'s 8-week journey begins to track progress accurately.
                    </p>
                    <StartDatePicker startDate={startDate} onStartDateChange={handleStartDateChange} />
                  </div>
                )}
                <div className="flex-1 p-4 border border-dashed rounded-lg">
                  <h3 className="font-medium mb-2">Add First Goal</h3>
                  <p className="text-sm text-muted-foreground mb-3">
                    Start by adding a goal to any week in {selectedAccount.name}'s timeline.
                  </p>
                  <Button asChild>
